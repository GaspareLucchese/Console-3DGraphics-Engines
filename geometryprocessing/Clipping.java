...
public class Clipping 
{
    ...
    public static List<Triangle> clipTriangleAgainstFrustum(Triangle tri, double[][] planes) 
    {
        ...
        for (int i = 0; i < planes.length; i++) 
        {
            ...
            clippedTriangles = newClippedTriangles; //Updating the list
            if (clippedTriangles.isEmpty()) 
            {
                break; //If the list is empty, it means the everything was deleted
            }
        }
    ...
    private static List<Triangle> clipTriangleAgainstPlane(Triangle tri, double[] plane) 
    {
        ...
        //We divide the vertices in inside points and outside points
        ...
        //[TO-DO] BUG DURING CLIPPING STAGE
        //If all the points are inside, we keep the triangle
        if (insidePoints.size() == 3) 
        {
            return List.of(tri);
        }
        //If we have a point inside and two outside, we create a new triangle (similar to the first one)
        ...
        //If we have a point inside and two outside, we create two new triangles from the first one
        ...
        //If all the points are outside, we delete the whole triangle (case insidePoints.size() == 0)
        else
        {
    ...
    private static Triangle clipOneInsideTwoOutside(Point3D inside, Point3D outside1, Point3D outside2, double[] plane) 
    {
        ...
        //[TO-DO] CHECK IF CLOCK-WISE SORTED
